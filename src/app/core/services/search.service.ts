...
export class SearchService {
  ...
  search(query: string): void {
    ...
    this.newsService.news().forEach(news => {
      const relevance = this.calculateRelevance(trimmed, news.title, news.excerpt, news.category);
      if (relevance > 0) {
  ...
  private calculateRelevance(query: string, title: string, description?: string, category?: string): number {
